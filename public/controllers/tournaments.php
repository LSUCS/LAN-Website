    ...
    class Tournaments_Controller extends LanWebsite_Controller {
        ...
        public function get_View($inputs) {
            ...
            if($tournament["team_size"] > 1) {
                ...
                $signups_list = $db->query("SELECT team_id, COUNT(user_id) AS players FROM `tournament_signups` WHERE tournament_id = '%s' GROUP BY team_id",
                                        $inputs['id'])->fetch_all(MYSQLI_ASSOC);
            }
            ...
            $tmpl = LanWebsite_Main::getTemplateManager();
            $tmpl->setSubTitle();
            $tmpl->addTemplate('tournament_view', array('tournament'=>$tournament, 'signup'=>$signup_bool, 'signup_list'=>$signups_list, 'brackets' => 0, 'matches', 0));
            $tmpl->output();
        }
    }

		...
		protected function process($user, $message) {
			...
			switch ($command) {
				...
				case "openconversation":
					...
					while ($row = $res->fetch_assoc()) {
						$res2 = LanWebsite_Main::getDb()->query("SELECT * FROM `chat_users` WHERE conversation_id = '%s'", $row["conversation_id"]);
                        if (mysqli_num_rows($res2) > 2) continue;
                        $exists = false;
                        while ($row2 = $res2->fetch_assoc()) if ($row2["user_id"] == $payload["userid"]) $exists = true;
						if ($exists) {

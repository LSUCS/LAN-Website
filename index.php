<?php
    ...
    if (LanWebsite_Main::isAtLan() && (!isset($_GET["page"]) || $_GET["page"] != "account")) {
        ...
        $user = LanWebsite_Main::$auth->getActiveUserData();
        
        //Ticket checks
        $ticket = self::$db->query("SELECT * FROM `tickets` WHERE assigned_forum_id = '%s' AND lan_number = '%s'", $user->getUserId(), LanWebsite_Main::getSettings()->getSetting("lan_number"))->fetch_assoc();

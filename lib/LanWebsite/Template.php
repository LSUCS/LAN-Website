<?php
    ...
    class LanWebsite_Template {
        ...
        private $plugins = array(
                                "jquery" => array( "scripts" => array( "/js/jquery.min.js", "/js/jquery-migrate.js" ), "styles" => array(), "include" => true ),
                                "cleditor" => array( "scripts" => array( "/js/jquery.cleditor.js" ), "styles" => array( "/css/jquery.cleditor.css" ), "include" => false ),
        ...
        public function addTemplate($template, $data = array()) {
            $this->template_stack[] = array("template" => $template, "data" => $data);
        }
        ...
        public function output() {
            ...
            if ($this->include_core) {
                ...
                $templates[] = array("template" => 'core_header', "data" => $data);
            }
            if ($this->include_main) {
                $data['title'] = ucwords($this->subtitle) . " | LSUCS LAN" . LanWebsite_Main::getSettings()->getSetting("lan_number");
                $data['refresh'] = $this->refresh;
                $templates[] = array("template" => 'header', "data" => $data);
            }
            
            //Body
            if ($this->include_core) {
                $templates[] = array("template" => 'core_body', "data" => array());
            }
            if ($this->include_main) {
                ...
                $data["styles"] = array();
                $data["scripts"] = array();
                $templates[] = array("template" => 'body', "data" => $data);
            }
            ...
            //Footer
            if ($this->include_core) {
                $templates[] = array("template" => 'core_footer', "data" => array());
            }
            if ($this->include_main) {
                $templates[] = array("template" => 'footer', "data" => array());
            }
            
            //Output templates
            foreach ($templates as $template) {
                $DataBag = $template['data'];
                include 'templates/' . $template['template'] . '.tmpl';
            }

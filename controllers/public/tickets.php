<?php
    ...
    class Tickets_Controller extends LanWebsite_Controller {
        ...
        public function get_Index() {
            ...
            $tmpl->addScript('/js/pages/tickets.js');
            $tmpl->addStyle('/css/pages/tickets.css');
            $tmpl->enablePlugin('spinner');
            $tmpl->addTemplate('public/tickets', $data);
			$tmpl->output();
        }
        ...
        public function post_Complete($inputs) {
            ...
            $tmpl->addScript('/js/pages/order-progress.js');
            $tmpl->addStyle('/css/pages/order-progress.css');
            $tmpl->addTemplate('public/order-progress', $data);
			$tmpl->output();
        }
        ...
        public function post_Checkcomplete($inputs) {
            ...
            //Retry
            else if ($purchase && !$purchase_error && $inputs["attempts"] <= LanWebsite_Main::getSettings()->getSetting("max_order_lookup_attempts")) {
        ...
        public function post_Checkout($inputs) {
            ...
            if ($result["availability"] < $inputs["member_amount"] + $inputs["nonmember_amount"]) {
        ...
        public function post_Ipn() {
            ...
            if ($verified) {
                ...
                if (empty($errmsg)) {
                    ...
                    // ISSUE RECEIPT
                    /********************/
                    $lanuser = lanWebsite_Main::getUserManager()->getUserById($pending_purchase["user_id"]);
                    $fields = array("api_key" => LanWebsite_Main::getSettings()->getSetting("api_key"),
                                    "lan" => LanWebsite_Main::getSettings()->getSetting("lan_number"),
                                    ...
                                    "email" => $user->getEmail(),
                                    "customer_forum_name" => $user->getUsername(),
                                    "student_id" => "");   
                    $fields_string = "";
                    foreach($fields as $key=>$value) $fields_string .= $key.'='.$value.'&';
                    rtrim($fields_string, '&');
